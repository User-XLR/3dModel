...
import SpecialAnnotationPanel from "@/components/special-annotation/SpecialAnnotationPanel";
...
export default class Viewer3DContainer extends Vue {
    ...
    for (let i = 0; i < proj.models.length; ++i) {
      ...
      viewer
        ...
        .then(() => {
          ...
          // 如果是IFC模型且所有模型都加载完成，自动打印IfcBuildingElementProxy对象
          if (counter === 0 && model.src.toLowerCase().endsWith(".ifc")) {
            setTimeout(() => {
              console.log(
                "模型加载完成，自动获取IfcBuildingElementProxy对象..."
              );
              this.printIfcBuildingElementProxies();
            }, 2000); // 延迟2秒确保模型完全加载
          }
        });
    }
   ...
   */
  printIfcBuildingElementProxies() {
    if (this.viewer) {
      console.log("开始获取并打印所有IfcBuildingElementProxy对象...");
      this.viewer.printAllIfcBuildingElementProxies();
    } else {
      Message.warning("Viewer未初始化");
    }
  }
   ...
   */
  analyzeIfcBuildingElementProxies() {
    if (this.viewer) {
      console.log("开始分析IfcBuildingElementProxy对象...");
      this.viewer.analyzeIfcBuildingElementProxies();
    } else {
      Message.warning("Viewer未初始化");
    }
  }
  ...
  protected render(): VNode {
    return (
      <div ref="viewerContainer" class={styles.viewerContainer}>
        ...
        <SpecialAnnotationPanel visible={true} />
        <BottomBar viewer={this.viewer} />

        {/* IFC代理对象调试按钮 */}
        <div class={styles.ifcDebugPanel}>
          <el-button
            type="primary"
            size="small"
            onClick={this.printIfcBuildingElementProxies}
            style="margin: 5px;"
          >
            打印IFC代理对象
          </el-button>
          <el-button
            type="success"
            size="small"
            onClick={this.analyzeIfcBuildingElementProxies}
            style="margin: 5px;"
          >
            分析IFC代理对象
          </el-button>
